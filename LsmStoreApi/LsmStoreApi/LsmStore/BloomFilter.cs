...
namespace LsmStoreApi.LsmStore
{
    ...
    public class Filter<T>
    {
        ...
        public void Add(T item)
        {
            ...
            int primaryHash = CalculateHash(Encoding.UTF8.GetBytes(item.ToString()));
        ...
        public void LoadFromBitString(string str)
        {
            var arr = str.Select(x => x == '1').ToArray();

            _hashBits = new BitArray(arr);
        }

        public void LoadFromBoolArray(bool[] arr)
        {
            _hashBits = new BitArray(arr);
        }

        public void Load(byte[] bits)
        {
            
            _hashBits = new BitArray(bits);
        }
    }
}
